...
class SystemService extends Service
{
	...
	public function getIpAddress($type = 'string')
	{
		...
		if($type == 'array') return $long ? array($ip, $long) : array('0.0.0.0', 0);
	...
	{
		if(!$user) return json_encode(array('ret'=>false, 'info'=>'用户信息获取失败!'));

        //获取上传图片服务器地址
        if($imgServer = $this->container->config['config.REMOTE_PIC_URL']){
            if(!$imgServer) return json_encode(array('ret'=>false, 'info'=>'获取图片服务器失败'));
        }
        ...
        //兼容表单方式上传
            ...
            if(!$files->getClientOriginalName()) return json_encode(array('ret'=>false, 'info'=>'上传图片错误'));
        ...
        //图片类型上传限制
        if(!in_array($image_extension, array('jpg','png', 'gif', 'jpeg'))) return json_encode(array('ret'=>false, 'info'=>'图片格式错误'));

		//单张照片大小1mb = 1000*1000
		if($image_size>1000000) return json_encode(array('ret'=>false, 'info'=>'图片上传超过限制大小'));

		//个人空间剩余判断
		if($image_size > $user['pic_total_size'] - $user['pic_used_size']) return json_encode(array('ret'=>false, 'info'=>'你的个人相册空间不足！'));


		//系统扩展判断
        if(!function_exists('curl_init')) return json_encode(array("ret"=> false,"info"=> '系统错误,不支持上传功能!'));
        ...
        $error = curl_error($ch);
		curl_close ($ch);
		if(!$result) return json_encode(array('ret'=>false, 'info'=>$error));
		return $result;
	}
}
