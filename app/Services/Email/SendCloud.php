...
class SendCloud {
	...
	protected function wrapParam_v2($mail) {
		$param = array ();
		if ($this->api_user) {
	...
	protected function wrapParam($mail) {
		if ($this->version == 'v1') {
	...
	protected function wrapParam_v1($mail) {
		$param = array ();
		if ($this->api_user) {
	...
	protected function wrapBody($mail) {
		if ($this->version == 'v1') {
	...
	protected function wrapBody_v1($mail) {
		$header = '';
	...
	protected function wrapBody_v2($mail) {
		$header = '';
		$paramArray = $mail->jsonSerialize ();
		$eol = "\r\n";
		$data = '';
	...
	public function sendCommon($mail) {
		global $config;
	...
	public function sendTemplate($mail) {
        $config = $this->config;
	    $method = "POST";
		if ($mail->hasAttachment ()) {
			$bodyData = $this->wrapBody ( $mail );
