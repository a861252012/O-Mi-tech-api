...
$len = count($domain);
ini_set('session.cookie_domain', '.'.$domain[$len-2].'.'.$domain[$len-1]);//限制二级域名
session_start();

if( !isset($_SESSION['room_is_login']) ){
    if( empty($_GET['uname']) || empty($_GET['passwd']) ){
        exit('input your username and password!');
    }
    //帐号密码可以修改
        if( $_GET['uname'] == '1roomadmin' && $_GET['passwd'] == '$1roompasswd' ){ //这里对应的帐号和密码都可以修改
        $_SESSION['room_is_login'] = true;
    }else{
        exit('your username or password is error!');
    }
}
