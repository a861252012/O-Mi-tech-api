...
$.fn.province_city_county = function(v_province,v_city,v_county){
    ...
    //获取省市数据
    $.get('/public/src/js/widget/ctry/xml/province_city.xml', function(data){
        var arrList = [];
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('图片 "' + file.name + '" 正在队列当中，需要替换原来已经上传的文件吗?');
					if (!replaceQueueItem) {
...
$(function() {
    ...
    var flashObj = ['<object width="689" height="506" type="application/x-shockwave-flash" data="/public/src/js/widget/avatar.swf" id="avatarflashNode">',
        '<param name="flashvars" value="saveUrl=/upload&amp;callback=flashEventHandler&amp;thumbList=150|80|40&amp;urlPic=/public/src/img/head_150.png">',
    ...
    if (!$.support.opacity) {
        flashObj = ['<object width="689" height="506" type="application/x-shockwave-flash" data="" id="avatarflashNode">',
                ...
                '<param name="allowScriptAccess" value="always">',
                '<param name="wmode" value="opaque">',
                '<param name="movie" value="/public/src/js/widget/avatar.swf">',
            '</object>'].join("");
    };
    ...
    $("#changeNickName").on("click",function () {
        $.dialog({
            title:"修改昵称",
            content:"<p>亲，有钱就是任性哦<br/>只需200钻石<br/>你就能获得一次修改昵称的特权哦</p>",
            okValue:"立即修改",
            ok:function () {
                $.ajax({
                    url:"/member/buyModifyNickname",
                    dataType:"json",
                    type:"post",
