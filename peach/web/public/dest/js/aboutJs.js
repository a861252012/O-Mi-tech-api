...
$(function(){
		...
		content: ['<div class="displaySuggestion">',
            	'<div class="display-item">',
            		'<label for="helpSelService">',
            			...
            			'<input id="helpSelService" class="txt txt-s d-help-ser-ipt" />',
            		'</label>',
            		'<label for="helpSelCat">',
            			'<span class="d-help-title">投诉与建议业务类别</span>',
            			'<select id="helpSelCat" class="select-s"><option>ob</option></select>',
            		'</label>',
            	'</div>',
	...
	var dialogFinish = $.dialog({
		...
		content: ['<div class="finishSuggestion">',
            	'亲爱的用户，感谢您宝贵的意见和建议，蜜桃儿将尽快处理该问题，并在3个工作日内以站内短信的方式回复您，请您注意查看哦！',
            '</div>'].join(""),
        cancel: function(){},
        cancelValue: "关闭"
	});
	...
	//点击我要扩建，显示dialog
	$(".help-title_ext").on("click", function(){
		dialogStrenth.show();
	});

	//点击帮助建议，显示dialog
	$(".help-title_sug").on("click", function(){
		dialogSug.show();
	});
