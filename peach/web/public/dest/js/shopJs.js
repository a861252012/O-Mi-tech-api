...
$(function(){
    ...
    var shoppingHTMLTmp = ['<div class="payContent clearfix">',
        '<div class="payleft">',
            '<ul class="shop-item payInfo">',
                '<li><img src="#{img}" alt="" class="payShowImg payImg"/></li>',
                '<li class="forange">#{name}</li>',
                '<li>价格：#{price}<img src="'+ Config.imagePath +'/diamond.png" alt="" />/月</li>',
            '</ul>',
            '<ul class="payTips">',
        ...
        '<div class="payright">',
            '<table class="table-form">',
                ...
                '<tr>',
                    '<th>购买方式：</th>',
                    '<td>',
                        '<button class="btn active pay-switch" id="payByMonth">按月付费</button>',
                        '<button class="btn btn-white pay-switch" id="payByYear">按年付费</button>',
                        '<img src="'+ Config.imagePath +'/discount.jpg" alt="" class="discount" />',
                    '</td>',
                '</tr>',
                ...
                '<tr>',
                    '<th>应付金额：</th>',
                    '<td class="bold"><span id="total-price">#{price}</span><img src="'+ Config.imagePath +'/diamond.png" alt="" /></td>',
                '</tr>',
                '<tr>',
                    '<th>当前余额：</th>',
                    '<td class="bold"><span id="current-price" class="fred">#{money}</span><img src="'+ Config.imagePath +'/diamond.png" alt="" /></td>',
                '</tr>',
                '<tr class="pay-tip">',
                    '<th></th>',
    ...
    //这个方法好神奇，需要改进。
    //总价变更
    //$ul: 商品列表， $timeIpt: 时间输入框
    var totalPrice = function($ul, $timeIpt){
        var btext = $('.pay-switch').filter(".active").text();
        if (btext === "按月付费") {
    ...
    //未登录处理
    var handleUnlogin = function(tipsText){
        $.dialog({
            title: "提示",
            content: tipsText,
            okValue: "立即登录",
            ...
            switch(data.code){
                ...
                case 0:
                    ...
                    $dateSwitch.on('click', function() {
                        //$payByMonth.find('button').removeClass('btn-red');
                        $(this).removeClass('btn-white').addClass("active");
                        $(this).siblings().addClass('btn-white').removeClass("active");
                        var btext = $(this).text();
                        if (btext === "按月付费") {
                            $dateTitle.text('个月');
                        }else{
                    ...
                    $payBtn.on('click',function(){
                        ...
                        $.ajax({
                            ...
                            data: {
                                gid: $ul.data('gid'),
                                nums: $timeIpt.val(),
                                type: $dateSwitch.index($dateSwitch.filter(".active"))
                            },
                            dataType: 'json',
                            type: "POST",
