...
//                $this->handleUncaughtException($handler);
    ...
    //多语言支持
    protected function registerLanguageBindings()
    {
        $this->lang = $this->make('request')->get("lang");
        $this->config['config.lang'] = $this->config['config.lang'] ?: [];
        if(in_array($this->lang,$this->config['config.lang'])){
            $_COOKIE['lang'] = $this->lang;
            setcookie('lang',  $this->lang);
        }else{
            $this->lang = ($this->make('request')->cookies->get('lang'));
            if(! in_array($this->lang,$this->config['config.lang']) ){
                $this->lang = $this->config['config.default_lang']?:'zh-cn';
                $_COOKIE['lang'] = $this->lang;
                setcookie('lang',  $this->lang);
            }
        }
        $temp_lang = $this->lang?:'zh-cn';
        $this->singleton('lang', function () use ($temp_lang) {
            $file = BASEDIR.'/Vcore/App/Config/'.$temp_lang.'.php';
            if(file_exists($file)){
                $lang = include($file);
            }else{
                throw new FileNotFoundException('Unable load language '. $this->lang. ' at '.$file);
            }
            return $lang;
        });
    ...
    public function goRun($request = null)
    {
        ...
        $GLOBALS['REMOTE_JS_URL'] = 'http://www' . $_W['v_remember_encrypt'];
    ...
    /**
     * 运行应用程序和发送响应 后台任务的核心部分
     *
     * @param  $argv |null $argv
     * @return void
     */
    public function goCron($argv = null)
    {
        $_W = include BASEDIR . '/Vcore/App/Config/config.php';  //database_host  database_port database_name database_user database_password
        $GLOBALS['CUR_DOMAIN'] = $_W['login_domain'][0];
        $GLOBALS['REMOTE_JS_URL'] = 'http://www' . $_W['login_domain'][0];

        try {
            // 根据匹配去调用路由  /task/end/(123)
            $this->handleDispatcherResponse(
            // 调用 fastRoute 类 进行匹配
                $this->createDispatcher()->dispatch("POST", $argv[1]));
        } catch (NotFoundHttpException $e) {
            //$handler = $this->make('ExceptionHandler');//异常信息处理
            //$handler->report($e);
            echo $e->getMessage();
            return;
        }
    ...
    public function dispatch($request = null)
    {
        if ($request) {
            ...
            $pathInfo = rtrim($request->getPathInfo(), '/') ? rtrim($request->getPathInfo(), '/') : '/';
        } else {
            $method = $this->getMethod();
            $pathInfo = $this->getPathInfo() ? $this->getPathInfo() : '/';
        }
        try {
    ...
    {
        $appConfigPath = ($this->configPath ?: $this->basePath('Vcore/App/Config')) . '/' . $name . '.php';
    ...
    protected function registerViewBindings()
    {
        ...
        $this->singleton('view', function () {
            ...
            $twig = new \Twig_Environment ($loader, array(
                'cache' => $this->config['config.debug']?false:$this->config['template.cache_dir'],// twig生成的缓存的路径
...
//            $twig->addGlobal('__IMAGE__', BASEDIR . '/App/Public/image');
            ...
            //http://s.1room1.com/public
            $twig->addGlobal('STATIC_PATH', $this->config['config.WEB_CDN_STATIC']);
            $twig->addGlobal('OPEN_WEB', $this->config['config.OPEN_WEB']);
            
            //http://10.1.100.194:4869
            $twig->addGlobal('IMG_PATH', $this->config['config.REMOTE_PIC_URL']);
            
            //http://p1.1room1.com/public
            $twig->addGlobal('PICTURE_CDN_PATH', $this->config['config.PIC_CDN_STATIC']);

            //Young 图片连接hack
            $twig->addGlobal('PIC_PATH', $this->config['config.PIC_PATH']);
            $twig->addGlobal('JS_PATH', $this->config['config.JS_PATH']);

            //Young 添加版本号
            $twig->addGlobal('publish_version', $redis->get('publish_version') ?: 'v2017090701');
            // 注册到twig中的自定义的函数 可以注册很多个

            // 注册url生成函数 U
            $url = new \Twig_SimpleFunction('U', array($this->make('url'), 'route'));
            $language = new \Twig_SimpleFunction('L', 'L');
    ...
    {
        // TODO 临时做了一个兼容原来配置的转换 以后要统一到database.php中
        $this->config['database'] = $this->config['config.database'];
    ...
    protected function registerLogBindings()
    {
        $this->singleton('log', function () {
            return new Logger('h', [
                (new StreamHandler(BASEDIR . '/Vcore/Cache/Logs/h_' . date('Y-m-d') . '.log', Logger::DEBUG))->setFormatter(new LineFormatter(null, null, true, true))
            ]);
        });
    }
    ...
    protected function registerRedisBindings()
    {
        $this->singleton('redis', function () {
            return new \Core\Redis\RedisService($this->config['config.redis']);
        });
