...
//            ->fetchAll('SELECT * FROM video_withdrawal_rules WHERE anchortype = '."'$type'".' AND minincome<='."'$points'".' and maxincome>='."'$points'");
    ...
    {
        return $headimg ? $this->container->config['config.REMOTE_PIC_URL'] . '/' . $headimg . ($size == 150 ? '' : '?w=' . $size . '&h=' . $size) : $this->container->config['config.WEB_CDN_STATIC'] . '/public/src/img/head_' . $size . '.png';
    }
